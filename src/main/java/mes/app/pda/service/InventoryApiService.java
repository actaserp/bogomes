...
public class InventoryApiService {
	...
	public List<Map<String, Object>> getMaterialCurrentStockList(String mat_type, Integer mat_grp_pk, String mat_name, Integer store_house_id, String spjangcd) {
		...
		String sql = """
			...
			""";
		if (org.apache.groovy.parser.antlr4.util.StringUtils.isEmpty(mat_type)==false) sql +=" and mg.\"MaterialType\" = :mat_type ";
		if (mat_grp_pk != null) sql +=" and mg.\"id\" = :mat_grp_pk ";
		if (org.apache.groovy.parser.antlr4.util.StringUtils.isEmpty(mat_name)==false) sql +=" and (m.\"Name\" like concat('%%',:mat_name,'%%') or m.\"Code\" like concat('%%',:mat_name,'%%') ) ";
