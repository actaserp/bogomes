		...
		String sql = """
				...
				shipment_summary AS (
				  SELECT
				   ...
				   CASE
				   	WHEN BOOL_OR(shp."state" = 'inspec') THEN 'inspec'
				   ...
				   END AS shipment_state
				   				  ...
				   				  FROM suju s
				     LEFT JOIN (
				       SELECT sh."SourceDataPk",
				              SUM(sh."Qty") AS "shippedQty",
				              MAX(shh."State") AS state
				       FROM shipment sh
				...
				vechidno_one AS (
				  SELECT
				    s."SujuHead_id",
				    MAX(shp.vechidno) AS vechidno
				  FROM suju s
				  JOIN shipment shp ON shp."SourceDataPk" = s.id
				  GROUP BY s."SujuHead_id"
				)
				SELECT
				  sh.id,
				...
				LEFT JOIN vechidno_one vo ON vo."SujuHead_id" = sh.id
